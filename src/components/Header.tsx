    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
      <nav className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          {/* Logo */}
            <a href="#home" className="flex items-center text-2xl font-bold gradient-text">
              <img src={logo} alt={"Logo"} className="logo-img w-7 h-7 mr-2" />
              Thomas BARBE
          </a>
          ...
          <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="sm">
                  {currentLanguage ? (
                    <div className="flex items-center transition-opacity duration-300">
                      <img src={currentLanguage.icon} alt={currentLanguage.label} className="w-5 h-5 mr-2" />
            ...
            <Button variant="ghost" size="icon" onClick={toggleTheme}>
              <div className="relative w-4 h-4">
                <Sun className={`absolute transition-all duration-300 ${theme === 'light' ? 'opacity-100 transform scale-100' : 'opacity-0 transform scale-0'}`} />
                <Moon className={`absolute transition-all duration-300 ${theme === 'dark' ? 'opacity-100 transform scale-100' : 'opacity-0 transform scale-0'}`} />
              </div>
            </Button>
